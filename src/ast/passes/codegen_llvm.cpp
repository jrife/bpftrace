...
ScopedExpr CodegenLLVM::visit(Call &call)
{
  if (call.func == "count") {
    ...
    if (ts_cast_) {
      b_.CreateStore(b_.CreateAdd(b_.CreateLoad(b_.getInt64Ty(), ts_cast_),
                                  b_.getInt64(1)),
                     ts_cast_);
    } else {
      b_.CreateMapElemAdd(
    ...
    if (ts_cast_) {
      BasicBlock *merge_block = BasicBlock::Create(module_->getContext(),
                                                   "merge_block",
                                                   parent);
      b_.createMinMax(b_.GetMapValueType(map.type),
                      ts_cast_,
                      expr,
                      map.type.IsSigned(),
                      is_max,
    ...
    Value *expr = b_.CreateIntCast(scoped_expr.value(),
                                   ...
                                   call.vargs.front()->type.IsSigned());
    if (ts_cast_) {
      b_.createAvg(b_.GetMapValueType(map.type), ts_cast_, expr);
    ...
    BasicBlock *action_block = BasicBlock::Create(module_->getContext(),
                                                  "action",
                                                  parent);
    Value *is_old_condition = b_.CreateICmpNE(epoch, epoch_val, "is_old_cond");
    ...
    //      avg:   [total:8, count:8,  epoch:8] - zero [0:127] clears total and
    //      count max:   [value:8, is_set:8, epoch:8] - zero [0:127] clears
    //      is_set min:   [value:8, is_set:8, epoch:8] - zero [0:127] clears
    //      is_set
    b_.CreateStore(
        b_.getInt64(0),
                                      ...
                                      const Location &loc)
{
  ...
  auto *key = b_.CreateMapKeyAllocation(CreateUInt64(),
                                        map.ident + "_key",
                                        loc);
