...
void Output::map_tseries_contents(
    ...
    const std::map<std::vector<uint8_t>,
                   std::map<uint64_t, std::vector<uint8_t>>> &values_by_key,
    uint64_t latest_epoch) const
{
  ...
  const auto &map_type = map_info.value_type;
  bool first = true;
  for (auto &key_value : values_by_key) {
    ...
    auto key_str = map_key_to_str(bpftrace, map, key);
    std::string val_str;
    auto &args = map_info.tseries_args;
    if (!args.has_value())
      LOG(BUG) << "call to tseries with missing arguments";
    val_str = tseries_to_str(bpftrace,
                             value,
                             latest_epoch,
                             args->interval_ns,
                             args->buckets,
...
void TextOutput::map_tseries(
    ...
    uint64_t latest_epoch) const
{
  map_tseries_contents(bpftrace, map, values_by_key, latest_epoch);
  out_ << std::endl;
}

template <typename T>
std::string tseries_to_str_text(
    BPFtrace &bpftrace,
    ...
    const SizedType &inner_type)
{
  ...
  for (auto &v : values) {
    ...
    if (epoch <= latest_epoch - buckets) {
      continue;
    }

    if (!min_epoch || epoch < min_epoch) {
      min_epoch = epoch;
    }
  }
  ...
  // Discover actual range of values.
  for (uint64_t epoch = min_epoch; epoch <= latest_epoch; epoch++) {
  ...
  if (min_value == max_value) {
    ...
    // in the graph, but don't overflow if our min or max is at the upper bound
    // of the range.
  ...
  if (interval_ns < 1000) {
    time_fmt += ".%k";
    time_legend += ".nnnnnnnnn";
  } else if (interval_ns < 1000000) {
    time_fmt += ".%f";
    time_legend += ".uuuuuu";
  } else if (interval_ns < 1000000000) {
    time_fmt += ".%l";
    time_legend += ".mmm";
  }
  ...
  for (uint64_t epoch = min_epoch; epoch <= latest_epoch; epoch++) {
    const auto &v = values.find(epoch);
    auto ts = bpftrace.resolve_timestamp(
        (uint32_t)TimestampMode::boot, epoch * interval_ns, time_fmt, false);
    ...
    if (v != values.end() || inner_type.IsSumTy() || inner_type.IsCountTy()) {
      T val = 0;
      if (v != values.end()) {
        val = util::read_data<T>(v->second.data());
      }
  ...
  if (zero_offset > 0) {
    bottom[zero_offset] = 'v';
  }
  res << std::setw(time_legend.length()) << "" << " " << bottom << std::endl;
  res << std::setw(time_legend.length()) << "" << " " << std::setw(21)
      << std::left << min_value << std::setw(graph_width - 21) << std::right
      << max_value << std::endl;

  return res.str();
}

std::string TextOutput::tseries_to_str(
    BPFtrace &bpftrace,
    const std::map<uint64_t, std::vector<uint8_t>> &values,
    uint64_t latest_epoch,
    uint64_t interval_ns,
    uint64_t buckets,
    const SizedType &inner_type) const
{
  if (inner_type.IsSigned()) {
    return tseries_to_str_text<int64_t>(
        bpftrace, values, latest_epoch, interval_ns, buckets, inner_type);
  } else {
...
template <typename T>
std::string tseries_to_str_json(
    BPFtrace &bpftrace,
    const std::map<uint64_t, std::vector<uint8_t>> &values,
    uint64_t latest_epoch,
    uint64_t interval_ns,
    ...
    const SizedType &inner_type)
{
  ...
  for (uint64_t epoch = min_epoch; epoch <= latest_epoch; epoch++) {
    ...
    auto ts = bpftrace.resolve_timestamp((uint32_t)TimestampMode::boot,
                                         epoch * interval_ns,
                                         "%FT%H:%M:%S.%kZ",
                                         true);
...
std::string JsonOutput::tseries_to_str(
    BPFtrace &bpftrace,
    const std::map<uint64_t, std::vector<uint8_t>> &values,
    uint64_t latest_epoch,
    uint64_t interval_ns,
