...
| n/a
| n/a
| The struct of all arguments of the traced function. Available in `tracepoint`, `fentry` and `fexit`.. Use `args.x` to access argument `x` or `args` to get a record with all arguments.
...
| <<map-functions-tseries, `tseries`>>
| Create a time series. tseries can be combined with `count`, `sum`, `avg`, `min`, or `max` to keep track of the count, sum, average, min, max of some value in each interval.
| Sync
...
[#map-functions-tseries]
=== tseries

.variants
* `tseries_t tseries(avg_t v, const string interval, int64 buckets)`
* `tseries_t tseries(count_t v, const string interval, int64 buckets)`
* `tseries_t tseries(sum_t v, const string interval, int64 buckets)`
* `tseries_t tseries(min_t v, const string interval, int64 buckets)`
* `tseries_t tseries(max_t v, const string interval, int64 buckets)`
* `tseries_t tseries(int64 v, const string interval, int64 buckets)`
* `tseries_t tseries(uint64 v, const string interval, int64 buckets)`

Create a time series that tracks some value. `tseries` records up to `buckets`
intervals where each bucket represents an amount of time specified by
`interval`. Its behavior depends on the type of `v`. To use `tseries`, the
<<enable_tseries>> config option must be enabled.

==== Map Types (`avg_t`, `count_t`, `sum_t`, `min_t`, `max_t`)

It is not legal to pass a named map of these types to `tseries`. Instead, a call
to `avg`, `count`, `sum`, `min`, or `max` must be embedded in the call to
`tseries`:

----
@ = tseries(sum(@c), "10s", 10); // This is OK
----

----
@s = sum(@c);
@ = tseries(@s, "10s", 10); // This is not OK
----

Combined with another map function, `tseries` keeps track of the average, count,
sum, min, or max of some value within each interval. In other words `tseries`
aggregates the values recorded in each interval using the specified function.

|===
| Map Function | Example | Description

| `avg`
| `@ = tseries(avg(@v), "1s", 5)`
| Calculate the running average of `@v` during each one second interval up to five seconds in the past.

| `count`
| `@ = tseries(count(), "1s", 5)`
| Count the number of times something happened during each one second interval up to five seconds in the past.

| `sum`
| `@ = tseries(sum(@v), "1s", 5)`
| Calculate the sum of all `@v` passed during each one second interval up to five seconds in the past.

| `min`
| `@ = tseries(min(@v), "1s", 5)`
| Calculate the minimum of all `@v` passed during each one second interval up to five seconds in the past.

| `min`
| `@ = tseries(max(@v), "1s", 5)`
| Calculate the maximum of all `@v` passed during each one second interval up to five seconds in the past.

|===

==== Integer Types (`int64`, `uint64`)

Simply keep track of the last value passed to `tseries` during each interval.

----
@c++;
@ = tseries(@c, "1s", 5); // Record the last value of @c passed to tseries during each interval.
----

==== Duration Strings

`interval` specifies the interval duration. It must be an unsigned integer
followed by a time unit suffix (`ns` (nanoseconds), `us` (microseconds), `ms` (
...
15:14:33.601 |                         .              o          | 3
15:14:33.701 |                         .                   o     | 4
             v_________________________v_________________________v
             -5                                                  5
----
